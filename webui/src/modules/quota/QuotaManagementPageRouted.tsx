...
import type { QuotaAdjustment, QuotaAlert, QuotaArchiveBatch, QuotaBillingRecord, QuotaBinding, QuotaBucketCandidate, QuotaBucketReplay, QuotaCreditOperation, QuotaCreditOperationInput, QuotaDailyRollup, QuotaGrant, QuotaPolicy, QuotaRoleCreditOperationInput, RbacRole } from "@/shared/types";
import { formatMicro } from "./QuotaUsagePage";

type Tab = "control" | "operations" | "recovery";
type ControlRoute = "policies" | "bindings" | "grants" | "adjustments";
...
function QuotaControlCenter({ route, onRouteChange, policies, bindings, grants, adjustments, activePolicies, policyForm, setPolicyForm, bindingForm, setBindingForm, grantForm, setGrantForm, adjustmentForm, setAdjustmentForm, savePolicy, saveBinding, saveGrant, saveAdjustment, publish, archivePolicy, retireBinding, revokeGrant }: { route: ControlRoute; onRouteChange: (route: ControlRoute) => void; policies: QuotaPolicy[]; bindings: QuotaBinding[]; grants: QuotaGrant[]; adjustments: QuotaAdjustment[]; activePolicies: QuotaPolicy[]; policyForm: PolicyForm; setPolicyForm: Dispatch<SetStateAction<PolicyForm>>; bindingForm: BindingForm; setBindingForm: Dispatch<SetStateAction<BindingForm>>; grantForm: AllocationForm; setGrantForm: Dispatch<SetStateAction<AllocationForm>>; adjustmentForm: AdjustmentForm; setAdjustmentForm: Dispatch<SetStateAction<AdjustmentForm>>; savePolicy: (policyId: string | null) => Promise<void>; saveBinding: () => Promise<void>; saveGrant: () => Promise<void>; saveAdjustment: () => Promise<void>; publish: (policyId: string) => Promise<void>; archivePolicy: (policyId: string) => Promise<void>; retireBinding: (bindingId: string) => Promise<void>; revokeGrant: (grantId: string) => Promise<void> }) { const content = route === "policies" ? <PolicyPanel policies={policies} form={policyForm} setForm={setPolicyForm} onSave={savePolicy} onPublish={publish} onArchive={archivePolicy} /> : route === "bindings" ? <BindingPanel bindings={bindings} activePolicies={activePolicies} form={bindingForm} setForm={setBindingForm} onSave={saveBinding} onRetire={retireBinding} /> : route === "grants" ? <GrantPanel grants={grants} form={grantForm} setForm={setGrantForm} onSave={saveGrant} onRevoke={revokeGrant} /> : <AdjustmentPanel adjustments={adjustments} form={adjustmentForm} setForm={setAdjustmentForm} onSave={saveAdjustment} />; return <><SubRouteNav value={route} onChange={(value) => onRouteChange(value as ControlRoute)} items={[{ value: "policies", label: "策略版本" }, { value: "bindings", label: "策略绑定" }, { value: "grants", label: "Grant 分配" }, { value: "adjustments", label: "手工调整" }]} /><div className="quota-route-panel">{content}</div></>; }
...
export function QuotaManagementPage() {
  ...
  const load = useCallback(async () => {
    ...
    const [policyResult, bindingResult, grantResult, adjustmentResult, creditResult, billingResult, alertResult, rollupResult, bucketResult, archiveResult] = results;
