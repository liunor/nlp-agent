...
import type { QuotaAdjustment, QuotaAlert, QuotaArchiveBatch, QuotaBillingRecord, QuotaBinding, QuotaBucketCandidate, QuotaBucketReplay, QuotaCreditOperation, QuotaCreditOperationInput, QuotaDailyRollup, QuotaGrant, QuotaMeterPricingRule, QuotaPolicy, QuotaPricingRule, QuotaRoleCreditOperationInput, RbacRole } from "@/shared/types";
import { formatMicro } from "./QuotaUsagePage";

type Tab = "control" | "operations" | "recovery";
type ControlRoute = "policies" | "pricing" | "meter_pricing" | "bindings" | "grants" | "adjustments";
...
function MeterPricingPanel() {
  const [rules, setRules] = useState<QuotaMeterPricingRule[]>([]);
  const [message, setMessage] = useState("");
  const [form, setForm] = useState({ capability_type: "search", meter: "search.requests", pricing_key: "", version: "", unit: "call", rate_micro: "", rate_unit: "1", min_charge_micro: "0", effective_from: localInput(), effective_until: "" });
  const loadRules = useCallback(async () => {
    try {
      const response = await api.listQuotaMeterPricingRules();
      setRules(response.items);
    } catch (error) {
      setMessage(reasonOf(error));
    }
  }, []);
  useEffect(() => { queueMicrotask(() => void loadRules()); }, [loadRules]);
  const save = async () => {
    const numericFields: Array<[string, string]> = [[form.rate_micro, "计价金额"], [form.rate_unit, "计价单位数量"], [form.min_charge_micro, "最低收费"]];
    if (!form.pricing_key.trim() || !form.version.trim() || !form.meter.trim() || numericFields.some(([value, label]) => policyNumberError(value, label))) return;
    try {
      await api.createQuotaMeterPricingRule({ ...form, pricing_key: form.pricing_key.trim(), meter: form.meter.trim(), version: form.version.trim(), rate_micro: Number(form.rate_micro), rate_unit: Number(form.rate_unit), min_charge_micro: Number(form.min_charge_micro), effective_from: toUtc(form.effective_from), effective_until: form.effective_until ? toUtc(form.effective_until) : null });
      setMessage("Meter 价格版本已创建");
      await loadRules();
    } catch (error) {
      setMessage(reasonOf(error));
    }
  };
  const retire = async (ruleId: string) => {
    try {
      await api.retireQuotaMeterPricingRule(ruleId);
      setMessage("Meter 价格版本已退役");
      await loadRules();
    } catch (error) {
      setMessage(reasonOf(error));
    }
  };
  return <section className="quota-panel quota-route-content"><SectionHeading icon={Coins} title="能力 Meter 价格" hint="为搜索、链接读取与 OCR 的标准计量项发布版本化整数价格。" /><div className="quota-form-grid"><Field label="能力类型"><select aria-label="Meter 能力类型" value={form.capability_type} onChange={(event) => setForm({ ...form, capability_type: event.target.value })}><option value="search">search</option><option value="web_fetch">web_fetch</option><option value="ocr">ocr</option></select></Field><Field label="Meter"><input aria-label="Meter" value={form.meter} onChange={(event) => setForm({ ...form, meter: event.target.value })} placeholder="search.requests" /></Field><Field label="Pricing Key"><input aria-label="Meter Pricing Key" value={form.pricing_key} onChange={(event) => setForm({ ...form, pricing_key: event.target.value })} placeholder="qwen/cn-beijing/web-search/turbo" /></Field><Field label="版本"><input aria-label="Meter 价格版本" value={form.version} onChange={(event) => setForm({ ...form, version: event.target.value })} placeholder="2026-09-03" /></Field><Field label="单位"><input aria-label="Meter 单位" value={form.unit} onChange={(event) => setForm({ ...form, unit: event.target.value })} placeholder="call" /></Field><Field label="每计价单位数量"><input aria-label="Meter 计价单位数量" inputMode="numeric" value={form.rate_unit} onChange={(event) => setForm({ ...form, rate_unit: event.target.value })} /></Field><Field label="计价金额（μcredits）"><input aria-label="Meter 计价金额" inputMode="numeric" value={form.rate_micro} onChange={(event) => setForm({ ...form, rate_micro: event.target.value })} /></Field><Field label="最低收费（μcredits）"><input aria-label="Meter 最低收费" inputMode="numeric" value={form.min_charge_micro} onChange={(event) => setForm({ ...form, min_charge_micro: event.target.value })} /></Field><Field label="生效时间"><input aria-label="Meter 生效时间" type="datetime-local" value={form.effective_from} onChange={(event) => setForm({ ...form, effective_from: event.target.value })} /></Field><Field label="结束时间（可选）"><input aria-label="Meter 结束时间" type="datetime-local" value={form.effective_until} onChange={(event) => setForm({ ...form, effective_until: event.target.value })} /></Field></div><p className="quota-form-help">每个 Meter 独立使用整数向上取整；发布后不可原地修改，只能退役并创建新版本。</p><button className="quota-primary-button" type="button" onClick={() => void save()} disabled={!form.pricing_key || !form.version || !form.rate_micro}><Save size={15} />创建 Meter 价格版本</button><Notice message={message} tone={message.includes("已") ? "success" : "error"} /><div className="quota-table-wrap"><table><thead><tr><th>Pricing Key / Meter</th><th>版本</th><th>整数费率</th><th>生效区间</th><th>状态</th><th>操作</th></tr></thead><tbody>{rules.map((rule) => <tr key={rule.id}><td><code>{rule.pricing_key}</code><small>{rule.meter} · {rule.unit}</small></td><td>{rule.version}</td><td>{rule.rate_micro.toLocaleString("zh-CN")} / {rule.rate_unit.toLocaleString("zh-CN")}<small>最低 {displayMicro(rule.minimum_charge_micro)}</small></td><td>{new Date(rule.effective_from).toLocaleString("zh-CN")} · {rule.effective_until ? new Date(rule.effective_until).toLocaleString("zh-CN") : "持续"}</td><td><span className={`quota-status-pill ${rule.status}`}>{rule.status}</span></td><td>{rule.status === "active" ? <button type="button" onClick={() => void retire(rule.id)}>退役</button> : <span className="quota-row-note">历史可追溯</span>}</td></tr>)}</tbody></table>{rules.length === 0 && <div className="quota-empty quota-empty-compact"><Coins size={18} />暂无能力 Meter 价格规则</div>}</div></section>;
}
...
function BillingCenter({ billing, onRefresh }: { billing: QuotaBillingRecord[]; onRefresh: () => Promise<void> }) {
  const [form, setForm] = useState({ provider: "", statement_id: "", operation_id: "", provider_response_id: "", usage_event_type: "model" as "model" | "capability", billed_usage: "", billed_at: localInput(), credits: "", idempotency_key: "" });
  const [repair, setRepair] = useState({ reason: "核对 Provider 账单后修复", idempotency_key: "" });
  const [message, setMessage] = useState("");
  const [saving, setSaving] = useState(false);
  const submit = async () => {
    setMessage("");
    if (!form.provider || !form.statement_id || (!form.operation_id && !form.provider_response_id) || !form.idempotency_key) { setMessage("Provider、账单号、operation_id/Provider Response ID 之一和幂等键均为必填项。"); return; }
    setSaving(true);
    try { const billedUsage = form.billed_usage.trim() ? JSON.parse(form.billed_usage) as Record<string, number> : {}; const result = await api.reconcileQuotaBilling({ provider: form.provider, statement_id: form.statement_id, operation_id: form.operation_id || null, provider_response_id: form.provider_response_id || null, usage_event_type: form.usage_event_type, billed_at: toUtc(form.billed_at), billed_credits_micro: form.credits ? Number(form.credits) : null, billed_tokens: {}, billed_usage: billedUsage, idempotency_key: form.idempotency_key }); setMessage(`已处理 ${result.total} 条账单：匹配 ${result.matched}，差异 ${result.discrepancies}，未匹配 ${result.unmatched}。`); await onRefresh(); } catch (reason) { setMessage(reasonOf(reason)); } finally { setSaving(false); }
  };
  ...
  return <section className="quota-panel quota-route-content"><SectionHeading icon={FileCheck2} title="Provider 账单对账" hint="优先按 operation_id 匹配，也可按 Provider Response ID 降级匹配。" /><div className="quota-form-grid"><Field label="Provider"><input value={form.provider} onChange={(event) => setForm({ ...form, provider: event.target.value })} placeholder="openai" /></Field><Field label="账单号"><input value={form.statement_id} onChange={(event) => setForm({ ...form, statement_id: event.target.value })} /></Field><Field label="事件类型"><select aria-label="账单事件类型" value={form.usage_event_type} onChange={(event) => setForm({ ...form, usage_event_type: event.target.value as "model" | "capability" })}><option value="model">model</option><option value="capability">capability</option></select></Field><Field label="operation_id"><input value={form.operation_id} onChange={(event) => setForm({ ...form, operation_id: event.target.value })} placeholder="与 Response ID 二选一" /></Field><Field label="Provider Response ID"><input value={form.provider_response_id} onChange={(event) => setForm({ ...form, provider_response_id: event.target.value })} placeholder="与 operation_id 二选一" /></Field><Field label="Provider Credits"><input inputMode="numeric" value={form.credits} onChange={(event) => setForm({ ...form, credits: event.target.value })} placeholder="未知可留空" /></Field><Field label="Meter 明细 JSON" className="quota-field-wide"><input value={form.billed_usage} onChange={(event) => setForm({ ...form, billed_usage: event.target.value })} placeholder={'{"search.requests": 1}'} /></Field><Field label="账单时间"><input type="datetime-local" value={form.billed_at} onChange={(event) => setForm({ ...form, billed_at: event.target.value })} /></Field><Field label="幂等键"><input value={form.idempotency_key} onChange={(event) => setForm({ ...form, idempotency_key: event.target.value })} /></Field></div><button className="quota-primary-button" type="button" onClick={() => void submit()} disabled={saving}><FileCheck2 size={15} />{saving ? "对账中…" : "提交账单对账"}</button><div className="quota-inline-tool-row"><Field label="账单修复原因"><input value={repair.reason} onChange={(event) => setRepair({ ...repair, reason: event.target.value })} /></Field><Field label="修复幂等键"><input value={repair.idempotency_key} onChange={(event) => setRepair({ ...repair, idempotency_key: event.target.value })} placeholder="每次修复唯一" /></Field></div><Notice message={message} tone={message.startsWith("已处理") ? "success" : "error"} /><div className="quota-table-wrap"><table><thead><tr><th>Provider / 账单</th><th>事件类型 / 匹配对象</th><th>Operation</th><th>Provider Credits / Meter</th><th>本地</th><th>差异</th><th>状态</th><th>操作</th></tr></thead><tbody>{billing.map((item) => <tr key={item.billing_id}><td>{item.provider}<small>{item.statement_id}</small></td><td>{item.usage_event_type || "model"}<small><code>{item.matched_capability_event_id || item.matched_usage_event_id || "未匹配"}</code></small></td><td><code>{item.operation_id}</code></td><td>{displayMicro(item.billed_credits_micro)}<small>{Object.entries(item.billed_usage || {}).map(([meter, quantity]) => `${meter}: ${quantity}`).join(" · ") || "无 Meter 明细"}</small></td><td>{displayMicro(item.local_credits_micro)}</td><td className={item.difference_micro ? "quota-value-warning" : ""}>{displayMicro(item.difference_micro)}</td><td><span className={`quota-status-pill ${item.status}`}>{item.status}</span></td><td>{item.status === "discrepancy" && <button type="button" onClick={() => void repairBilling(item)}>修复</button>}</td></tr>)}</tbody></table>{billing.length === 0 && <div className="quota-empty quota-empty-compact"><FileCheck2 size={18} />暂无账单记录</div>}</div></section>;
}
