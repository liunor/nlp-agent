...
function TopicDetailView({ topic, onBack, onChange, onRemove, onAddPoint, onEditPoint }: { topic: CourseTopic; onBack: () => void; onChange: (topic: CourseTopic) => void; onRemove: () => void; onAddPoint: () => void; onEditPoint: (pointId: string) => void }) {
  ...
  return <section className="teacher-panel">
    <header className="teacher-detail-bar"><button className="teacher-secondary-button" type="button" onClick={onBack}><ChevronLeft size={15} />返回主题目录</button><div className="teacher-detail-title"><BookOpen size={17} /><h2>{topic.name || "未命名主题"}</h2><StatusBadge status={topic.status} /></div><button className="teacher-primary-button" type="button" onClick={onAddPoint}><Plus size={16} />新建知识点</button></header>
    <div className="teacher-catalog-body">
      <div className="teacher-editor-grid teacher-inset-grid">
        ...
        <div className="teacher-editor-actions"><StatusButton status={topic.status} onChange={(status) => onChange({ ...topic, status })} /><RemoveButton label={`主题“${topic.name || "未命名"}”`} onRemove={onRemove} /></div>
      </div>
...
function PointEditorView({ point, onBack, onSave, onRemove }: { point: KnowledgePoint; onBack: () => void; onSave: (point: KnowledgePoint) => void; onRemove: () => void }) {
  ...
  return <section className="teacher-panel">
    <header className="teacher-detail-bar"><button className="teacher-secondary-button" type="button" onClick={onBack}><ChevronLeft size={15} />返回知识点列表</button><div className="teacher-detail-title"><BookOpen size={16} /><h2>{draft.name || "未命名知识点"}</h2></div><StatusBadge status={draft.status} /></header>
    <div className="teacher-editor-grid"><label>知识点名称<input value={draft.name} onChange={(event) => edit({ name: event.target.value })} placeholder="例如：缩放点积注意力" /></label><label>知识点 Markdown<textarea rows={12} value={draft.markdown} onChange={(event) => edit({ markdown: event.target.value })} placeholder="用 Markdown 写清概念边界、必须覆盖的内容与容易混淆处。" /></label><p className="teacher-field-hint">启用的知识点会成为该主题的回答范围约束；可使用公式、表格、代码和 Mermaid。</p><div className="teacher-editor-actions"><StatusButton status={draft.status} onChange={(status) => edit({ status })} /><RemoveButton label={`知识点“${draft.name || "未命名"}”`} onRemove={onRemove} /><button className="teacher-primary-button" type="button" onClick={() => { onSave(draft); setSaved(true); }}><Save size={15} />保存知识点</button>{saved && <span role="status">已保存到当前目录；点击下方“保存教学目录”同步到后端。</span>}</div></div>
  </section>;
}
