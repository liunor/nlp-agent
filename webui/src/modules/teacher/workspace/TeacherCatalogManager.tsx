import { BookOpen, ChevronLeft, ChevronRight, Eye, FilePlus2, MessageCircleQuestion, PanelsTopLeft, Pencil, Plus, Power, Save, Trash2 } from "lucide-react";
...
function MarkdownEditorWithPreview({ value, onChange, placeholder, rows = 7, label, ariaLabel }: { value: string; onChange: (value: string) => void; placeholder?: string; rows?: number; label: string; ariaLabel?: string }) {
  const [mode, setMode] = useState<"edit" | "split" | "preview">("split");
  const deferredValue = useDeferredValue(value);
  return (
    <div className="teacher-markdown-field">
      <div className="teacher-markdown-field-header">
        <span>{label}</span>
        <div className="teacher-markdown-tabs" role="tablist" aria-label="Markdown 预览切换">
          <button type="button" role="tab" aria-selected={mode === "edit"} className={mode === "edit" ? "active" : ""} onClick={() => setMode("edit")}><Pencil size={12} />编辑</button>
          <button type="button" role="tab" aria-selected={mode === "split"} className={mode === "split" ? "active" : ""} onClick={() => setMode("split")}><PanelsTopLeft size={12} />分栏</button>
          <button type="button" role="tab" aria-selected={mode === "preview"} className={mode === "preview" ? "active" : ""} onClick={() => setMode("preview")}><Eye size={12} />预览</button>
        </div>
      </div>
      <div className={`teacher-markdown-split mode-${mode}`}>
        {(mode === "edit" || mode === "split") && (
          <textarea value={value} onChange={(event) => onChange(event.target.value)} placeholder={placeholder} rows={rows} aria-label={ariaLabel ?? label} />
        )}
        {(mode === "preview" || mode === "split") && (
          <div className="teacher-markdown-preview" role="region" aria-label="Markdown 预览">
            {deferredValue.trim() ? <MarkdownContent>{deferredValue}</MarkdownContent> : <span className="teacher-markdown-preview-empty">暂无内容，输入 Markdown 后将在此预览</span>}
          </div>
        )}
      </div>
    </div>
...
function PointEditorView({ point, onBack, onSave, onRemove, onDirtyChange }: { point: KnowledgePoint; onBack: () => void; onSave: (point: KnowledgePoint) => void; onRemove: () => void; onDirtyChange?: (dirty: boolean) => void }) {
  const [draft, setDraft] = useState(point); const [saved, setSaved] = useState(false);
  const edit = (patch: Partial<KnowledgePoint>) => { setDraft({ ...draft, ...patch }); setSaved(false); onDirtyChange?.(true); };
  return <section className="teacher-panel">
    <header className="teacher-detail-bar"><button className="teacher-secondary-button" type="button" onClick={onBack}><ChevronLeft size={15} />返回知识点列表</button><div className="teacher-detail-title"><BookOpen size={16} /><h2>{draft.name || "未命名知识点"}</h2><StatusBadge status={draft.status} /></div><div className="teacher-detail-actions"><StatusButton status={draft.status} onChange={(status) => edit({ status })} /><RemoveButton label={`知识点“${draft.name || "未命名"}”`} onRemove={() => { onDirtyChange?.(false); onRemove(); }} /><button className="teacher-primary-button" type="button" onClick={() => { onSave(draft); setSaved(true); onDirtyChange?.(false); }}><Save size={15} />保存知识点</button></div></header>
    <div className="teacher-editor-grid"><label>知识点名称<input value={draft.name} onChange={(event) => edit({ name: event.target.value })} placeholder="例如：缩放点积注意力" /></label><MarkdownEditorWithPreview label="知识点 Markdown" value={draft.markdown} onChange={(value) => edit({ markdown: value })} placeholder="用 Markdown 写清概念边界、必须覆盖的内容与容易混淆处。" rows={12} /><p className="teacher-field-hint">启用的知识点会成为该主题的回答范围约束；可使用公式、表格、代码和 Mermaid。右侧可实时预览渲染效果。</p>{saved && <p className="teacher-field-hint" role="status">已保存到当前目录；点击下方“保存教学目录”同步到后端。</p>}</div>
  </section>;
}

export function TopicCatalogEditor({ topics, onChange, onDirtyChange }: { topics: CourseTopic[]; onChange: (topics: CourseTopic[]) => void; onDirtyChange?: (dirty: boolean) => void }) {
  ...
  const [pendingPointLeave, setPendingPointLeave] = useState<(() => void) | null>(null);
  const handlePointDirtyChange = (dirty: boolean) => { setPointDirty(dirty); };
  const createDirty = !!name.trim() || !!description.trim();
  const combinedDirty = pointDirty || createDirty;
  useEffect(() => {
    onDirtyChange?.(combinedDirty);
  }, [combinedDirty, onDirtyChange]);
  const requestPointBack = (next: () => void) => {
    if (pointDirty) { setPendingPointLeave(() => next); return; }
    next();
  };
  ...
  const removePoint = (topicId: string, pointId: string) => { const topic = topics.find((item) => item.id === topicId); if (!topic) return; update({ ...topic, knowledge_points: topic.knowledge_points.filter((item) => item.id !== pointId) }); setPointDirty(false); setView({ kind: "topic", topicId }); };
  ...
  return <div className="teacher-stack">
    ...
    {pointView && editPoint && <PointEditorView key={editPoint.id} point={editPoint} onBack={() => requestPointBack(() => { setPointDirty(false); setView({ kind: "topic", topicId: pointView.topicId }); })} onSave={(point) => updatePoint(pointView.topicId, point)} onRemove={() => removePoint(pointView.topicId, editPoint.id)} onDirtyChange={handlePointDirtyChange} />}
    <ConfirmDialog open={pendingPointLeave !== null} title="有未保存的知识点修改" description="当前知识点修改尚未保存到目录，返回后修改将丢失。" confirmLabel="放弃修改并返回" cancelLabel="继续编辑" onClose={() => setPendingPointLeave(null)} onConfirm={() => { const next = pendingPointLeave; setPointDirty(false); setPendingPointLeave(null); next?.(); }} />
  </div>;
}
...
function BlueprintEditorView({ kind, item, topics, exerciseBlueprints, onBack, onSave, onRemove, onDirtyChange }: { kind: BlueprintKind; item: Blueprint; topics: CourseTopic[]; exerciseBlueprints: ExerciseBlueprint[]; onBack: () => void; onSave: (item: Blueprint) => void; onRemove: () => void; onDirtyChange?: (dirty: boolean) => void }) {
  const [draft, setDraft] = useState(item); const [saved, setSaved] = useState(false);
  const edit = (patch: Partial<Blueprint>) => { setDraft({ ...draft, ...patch } as Blueprint); setSaved(false); onDirtyChange?.(true); };
  ...
  return <section className="teacher-panel">
    ...
    <div className="teacher-editor-grid">
      ...
      <MarkdownEditorWithPreview label="单题 Markdown 指令" value={draft.instructions} onChange={(value) => edit({ instructions: value })} placeholder="只描述这一道题的题干范围、数据要求与讲评规则。" rows={7} />
      <RubricEditor rubric={draft.rubric} onChange={(rubric) => edit({ rubric })} />
      <div className="teacher-editor-actions"><StatusButton status={draft.status} onChange={(status) => edit({ status })} /><RemoveButton label={`${blueprintName(kind)}“${draft.name || "未命名"}”`} onRemove={() => { onDirtyChange?.(false); onRemove(); }} /><button className="teacher-primary-button" type="button" onClick={() => { onSave(draft); setSaved(true); onDirtyChange?.(false); }}><Save size={15} />保存蓝图</button>{saved && <span role="status">已保存到当前目录；点击下方“保存教学目录”同步到后端。</span>}</div>
    </div>
  </section>;
}

export function BlueprintCatalogEditor({ kind, topics, blueprints, exerciseBlueprints, onChange, onDirtyChange }: { kind: BlueprintKind; topics: CourseTopic[]; blueprints: Blueprint[]; exerciseBlueprints: ExerciseBlueprint[]; onChange: (items: Blueprint[]) => void; onDirtyChange?: (dirty: boolean) => void }) {
  ...
  const [pendingBlueprintLeave, setPendingBlueprintLeave] = useState<(() => void) | null>(null);
  const handleBlueprintDirtyChange = (dirty: boolean) => { setBlueprintDirty(dirty); };
  const createDirty = !!name.trim() || !!topicId || !!pointId || !!instructions.trim() || questionType !== "简答";
  const combinedBlueprintDirty = createDirty || blueprintDirty;
  useEffect(() => {
    onDirtyChange?.(combinedBlueprintDirty);
  }, [combinedBlueprintDirty, onDirtyChange]);
  const requestBlueprintBack = (next: () => void) => {
    if (blueprintDirty) { setPendingBlueprintLeave(() => next); return; }
    next();
  };
  ...
  return <div className="teacher-stack">
    ...
    {editing && <BlueprintEditorView key={editing.id} kind={kind} item={editing} topics={topics} exerciseBlueprints={exerciseBlueprints} onBack={() => requestBlueprintBack(() => { setBlueprintDirty(false); setEditingId(""); })} onSave={(next) => onChange(blueprints.map((item) => item.id === next.id ? next : item))} onRemove={() => { setBlueprintDirty(false); onChange(blueprints.filter((item) => item.id !== editing.id)); setEditingId(""); }} onDirtyChange={handleBlueprintDirtyChange} />}
    <ConfirmDialog open={pendingBlueprintLeave !== null} title="有未保存的蓝图修改" description="当前蓝图修改尚未保存到目录，返回后修改将丢失。" confirmLabel="放弃修改并返回" cancelLabel="继续编辑" onClose={() => setPendingBlueprintLeave(null)} onConfirm={() => { const next = pendingBlueprintLeave; setBlueprintDirty(false); setPendingBlueprintLeave(null); next?.(); }} />
  </div>;
}
...
function GuidedBlueprintEditorView({ item, topics, onBack, onSave, onRemove, onDirtyChange }: { item: GuidedBlueprint; topics: CourseTopic[]; onBack: () => void; onSave: (item: GuidedBlueprint) => void; onRemove: () => void; onDirtyChange?: (dirty: boolean) => void }) {
  const [draft, setDraft] = useState(item); const [saved, setSaved] = useState(false);
  const edit = (patch: Partial<GuidedBlueprint>) => { setDraft({ ...draft, ...patch }); setSaved(false); onDirtyChange?.(true); };
  const allowedPoints = topics.find((value) => value.id === draft.topic_id)?.knowledge_points ?? [];
  return <section className="teacher-panel">
    ...
    <div className="teacher-editor-grid">
      ...
      <MarkdownEditorWithPreview label="引导方向（Markdown 指令）" value={draft.guidance} onChange={(value) => edit({ guidance: value })} placeholder="说明教师希望模型采用的引导路径、提问顺序和应聚焦的误区。" rows={7} />
      <div className="teacher-editor-actions"><StatusButton status={draft.status} onChange={(status) => edit({ status })} /><RemoveButton label={`引导蓝图“${draft.name || "未命名"}”`} onRemove={() => { onDirtyChange?.(false); onRemove(); }} /><button className="teacher-primary-button" type="button" onClick={() => { onSave(draft); setSaved(true); onDirtyChange?.(false); }}><Save size={15} />保存蓝图</button>{saved && <span role="status">已保存到当前目录；点击下方“保存教学目录”同步到后端。</span>}</div>
    </div>
  </section>;
}

export function GuidedBlueprintCatalogEditor({ topics, blueprints = [], onChange, onDirtyChange }: { topics: CourseTopic[]; blueprints?: GuidedBlueprint[]; onChange: (items: GuidedBlueprint[]) => void; onDirtyChange?: (dirty: boolean) => void }) {
  ...
  const [pendingGuidedLeave, setPendingGuidedLeave] = useState<(() => void) | null>(null);
  const handleGuidedDirtyChange = (dirty: boolean) => { setGuidedDirty(dirty); };
  const createGuidedDirty = !!name.trim() || !!topicId || !!pointId || !!guidance.trim();
  const combinedGuidedDirty = createGuidedDirty || guidedDirty;
  useEffect(() => {
    onDirtyChange?.(combinedGuidedDirty);
  }, [combinedGuidedDirty, onDirtyChange]);
  const requestGuidedBack = (next: () => void) => {
    if (guidedDirty) { setPendingGuidedLeave(() => next); return; }
    next();
  };
  const topicOptions = topics.filter((topic) => topic.status !== "disabled"); const selectedTopic = topicOptions.find((topic) => topic.id === topicId); const points = selectedTopic?.knowledge_points.filter((point) => point.status === "enabled") ?? [];
  const editing = blueprints.find((item) => item.id === editingId);
  ...
  return <div className="teacher-stack">
    ...
    {editing && <GuidedBlueprintEditorView key={editing.id} item={editing} topics={topics} onBack={() => requestGuidedBack(() => { setGuidedDirty(false); setEditingId(""); })} onSave={(next) => onChange(blueprints.map((item) => item.id === next.id ? next : item))} onRemove={() => { setGuidedDirty(false); onChange(blueprints.filter((item) => item.id !== editing.id)); setEditingId(""); }} onDirtyChange={handleGuidedDirtyChange} />}
    <ConfirmDialog open={pendingGuidedLeave !== null} title="有未保存的引导蓝图修改" description="当前引导蓝图修改尚未保存到目录，返回后修改将丢失。" confirmLabel="放弃修改并返回" cancelLabel="继续编辑" onClose={() => setPendingGuidedLeave(null)} onConfirm={() => { const next = pendingGuidedLeave; setGuidedDirty(false); setPendingGuidedLeave(null); next?.(); }} />
  </div>;
}
