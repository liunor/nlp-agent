...
type ModelProfileDraft = { label: string; provider: string; coordinator: string; worker: string; utility: string };
...
function modelProfileDraft(value: unknown = {}): ModelProfileDraft {
  const source = asRecord(value);
  return {
    label: String(source.label ?? ""),
    provider: String(source.provider ?? ""),
    coordinator: String(source.coordinator ?? ""),
    worker: String(source.worker ?? ""),
...
export function Models({ snapshot, refresh }: { snapshot: DeveloperSnapshot; refresh: () => Promise<void> }) {
  ...
  const profileNames = Object.keys(snapshot.models.profiles);
  const [view, setView] = useState<"providers" | "presets" | "routes" | "profiles">("providers");
  ...
  const [profileName, setProfileName] = useState(profileNames[0] ?? "");
  const [profileDraft, setProfileDraft] = useState<ModelProfileDraft>(() => modelProfileDraft(snapshot.models.profiles[profileNames[0]]));
  const [message, setMessage] = useState("");
  ...
  const selectProfile = (name: string) => { setProfileName(name); setProfileDraft(modelProfileDraft(snapshot.models.profiles[name])); setMessage(""); };
  const providerForPreset = (name: string) => {
    const preset = asRecord(snapshot.models.presets[name]);
    const model = asRecord(snapshot.models.models[String(preset.model ?? "")]);
    return String(model.provider ?? "");
  };
  const profilePresetNames = presetNames.filter((name) => providerForPreset(name) === profileDraft.provider);
  const selectProfileProvider = (provider: string) => {
    const availablePresets = presetNames.filter((name) => providerForPreset(name) === provider);
    setProfileDraft((current) => ({
      ...current,
      provider,
      coordinator: availablePresets.includes(current.coordinator) ? current.coordinator : availablePresets[0] ?? "",
      worker: availablePresets.includes(current.worker) ? current.worker : availablePresets[0] ?? "",
      utility: availablePresets.includes(current.utility) ? current.utility : availablePresets[0] ?? "",
    }));
  };
  ...
  const saveProfile = async () => { try { await api.saveModelProfile(profileName, profileDraft); setMessage("模型档案已保存，新的会话可按此档案选择模型"); await refresh(); } catch (error) { setMessage(error instanceof Error ? error.message : String(error)); } };
  return <div className="developer-control-page">
    ...
    {view === "profiles" && <section className="developer-model-editor developer-model-single"><div className="developer-control-section-heading"><div><h2>模型档案</h2><p>把同一 Provider 下的 Coordinator、Worker、Utility 预设组合起来，学生端会按会话选择它。</p></div>{profileName === snapshot.models.default_model_profile && <StatusPill ok>默认档案</StatusPill>}</div>{profileNames.length === 0 ? <p className="developer-control-empty">当前没有可编辑的模型档案。</p> : <><label className="developer-control-field">选择档案<select aria-label="模型档案" value={profileName} onChange={(event) => selectProfile(event.target.value)}>{profileNames.map((name) => <option key={name} value={name}>{String(asRecord(snapshot.models.profiles[name]).label ?? name)} · {name}</option>)}</select></label>{profileName && <><div className="developer-control-field-grid"><label className="developer-control-field">显示名称<input aria-label={`${profileName} 显示名称`} value={profileDraft.label} onChange={(event) => setProfileDraft((current) => ({ ...current, label: event.target.value }))} /></label><label className="developer-control-field">Provider<select aria-label={`${profileName} Provider`} value={profileDraft.provider} onChange={(event) => selectProfileProvider(event.target.value)}>{providerNames.map((name) => <option key={name} value={name}>{name}</option>)}</select></label><label className="developer-control-field">Coordinator 预设<select aria-label={`${profileName} Coordinator 预设`} value={profileDraft.coordinator} onChange={(event) => setProfileDraft((current) => ({ ...current, coordinator: event.target.value }))}>{profilePresetNames.map((name) => <option key={name} value={name}>{name}</option>)}</select></label><label className="developer-control-field">Worker 预设<select aria-label={`${profileName} Worker 预设`} value={profileDraft.worker} onChange={(event) => setProfileDraft((current) => ({ ...current, worker: event.target.value }))}>{profilePresetNames.map((name) => <option key={name} value={name}>{name}</option>)}</select></label><label className="developer-control-field">Utility 预设<select aria-label={`${profileName} Utility 预设`} value={profileDraft.utility} onChange={(event) => setProfileDraft((current) => ({ ...current, utility: event.target.value }))}>{profilePresetNames.map((name) => <option key={name} value={name}>{name}</option>)}</select></label></div><div className="developer-control-actions"><button className="developer-control-primary" type="button" onClick={() => void saveProfile()} disabled={profilePresetNames.length === 0}>保存模型档案</button>{message && <small role="status">{message}</small>}</div></>}</>}</section>}
  </div>;
}
