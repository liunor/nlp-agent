...
def upgrade() -> None:
    ...
    if "phone_number_normalized" not in columns:
        ...
        rows = bind.execute(sa.text("SELECT id, phone_number FROM nlp_users WHERE phone_number IS NOT NULL")).fetchall()
        for user_id, phone in rows:
            normalized = _legacy_normalize(phone)
            if normalized:
                bind.execute(sa.text("UPDATE nlp_users SET phone_number_normalized=:phone WHERE id=:id"), {"phone": normalized, "id": user_id})
        ...
        duplicates = bind.execute(sa.text("SELECT phone_number_normalized FROM nlp_users WHERE phone_number_normalized IS NOT NULL GROUP BY phone_number_normalized HAVING COUNT(*) > 1")).fetchall()
        for (phone,) in duplicates:
            ids = bind.execute(sa.text("SELECT id FROM nlp_users WHERE phone_number_normalized=:phone ORDER BY created_at, id"), {"phone": phone}).fetchall()
            for (user_id,) in ids[1:]:
                bind.execute(sa.text("UPDATE nlp_users SET phone_number_normalized=NULL WHERE id=:id"), {"id": user_id})
    unique_names = {item.get("name") for item in sa.inspect(bind).get_unique_constraints("nlp_users")}
    if "uq_nlp_users_phone_number_normalized" not in unique_names:
        op.create_unique_constraint("uq_nlp_users_phone_number_normalized", "nlp_users", ["phone_number_normalized"])
    index_names = {item.get("name") for item in sa.inspect(bind).get_indexes("nlp_users")}
...
def downgrade() -> None:
    bind = op.get_bind()
    if "nlp_sms_send_audits" in sa.inspect(bind).get_table_names():
