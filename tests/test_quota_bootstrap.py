...
def test_quota_schema_verification_probes_provider_cache_miss_column():
    engine = create_engine("sqlite:///:memory:")
    Base.metadata.create_all(
        engine,
        tables=[
    ...
    with engine.begin() as connection:
        connection.exec_driver_sql(
            "ALTER TABLE nlp_usage_events DROP COLUMN cache_miss_input_tokens"
        )

    with pytest.raises(OperationalError, match="cache_miss_input_tokens"):
        QuotaService(engine).verify_schema()


def test_required_usage_reporter_verifies_the_complete_usage_event_schema():
    from server.quota import bootstrap

    engine = create_engine("sqlite:///:memory:")
    UsageEventModel.__table__.create(engine)
    with engine.begin() as connection:
        connection.exec_driver_sql(
            "ALTER TABLE nlp_usage_events DROP COLUMN cache_miss_input_tokens"
        )
    ...
    with pytest.raises(OperationalError, match="cache_miss_input_tokens"):
        bootstrap.configure_usage_reporter(engine, required=True)
