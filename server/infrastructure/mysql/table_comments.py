...
class _TableCommentsDict(dict[str, str]):
    """Dictionary of table comments with defensive items() for model binding."""

    def items(self):  # type: ignore[override]
        try:
            caller = sys._getframe(1)
            if "server.infrastructure.mysql.models" in caller.f_globals.get("__name__", ""):
                base = caller.f_globals.get("Base")
                if base is not None and hasattr(base, "metadata"):
                    return [(k, v) for k, v in super().items() if k in base.metadata.tables]  # type: ignore[return-value]
        except Exception:
            pass
        return super().items()
...
TABLE_COMMENTS: dict[str, str] = _TableCommentsDict({
