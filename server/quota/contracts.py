...
ReservationStatus = Literal[
    "reserved", "running", "settling", "settled", "released", "expired", "completed"
]
...
class PolicyBinding(QuotaFrozenModel):
    ...
    @classmethod
    def require_utc(cls, value: datetime | None, info: ValidationInfo) -> datetime | None:
...
class FinishTurn(QuotaFrozenModel):
    ...
    actual_credits_micro: StrictNonNegativeInt | None = None
...
class Reservation(QuotaFrozenModel):
    ...
    def validate_terminal_state(self) -> "Reservation":
        if self.status in {"settled", "released", "expired", "completed"} and self.reserved_micro != 0:
