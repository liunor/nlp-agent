...
- **验证逻辑**:
  - 验证图片验证码
  - 验证短信验证码（单次消费、120 秒过期）
  - 检查手机号和规范化用户名是否已存在
  - 通过 `UserService.register_user()` 创建用户、个人工作空间和默认 `guest` 角色
...
- **验证码存储与校验（server/auth/code_store.py）**:
  - 答案哈希后存入 `nlp_auth_codes` 表（多实例共享）
  - 不区分大小写比较（哈希前 casefold）
  - 一次性使用（消费即删，无法重放）
  - 消费时强制检查过期时间（2 分钟 TTL）
